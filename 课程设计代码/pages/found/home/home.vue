			...
			>
				<swiper-item v-for="tabItem in tabBars" :key="tabItem.id">
					<scroll-view 
						...
						-->
						<view v-for="(item, index) in tabItem.newsList" 
						:key="index"
						class="news-item" 
	...
	export default {
		...
		computed: {
			advertNavUrl(){
				...
				return `/pages/found/details/details?data=${JSON.stringify(data)}`;
			} 
		},
		async onLoad() {
			// 获取屏幕宽度
		...
		methods: {
			...
			navToDetails(item){
				...
				//是否有videoSrc属性，有则跳转videoDetails，没有则跳转details
				let url = item.videoSrc ? 'videoDetails' : 'details'; 

				uni.navigateTo({
					url: `/pages/found/details/${url}?data=${JSON.stringify(data)}`
				})
	...
	.nav-bar{
		...
		.nav-item{
			...
			&:after{
				...
				width: 0;
				height: 0;
				border-bottom: 4upx solid #22c993;
				position: absolute;
				left: 50%;
				bottom: 0;
